...
默认配置会启用 PaddleOCR 进行车牌文本识别与日志记录，请在创建完虚拟环境后安装以下依赖（`requirements.txt` 已列出，可直接 `pip install -r requirements.txt`）：

```bash
pip install paddleocr>=2.7.0
# Linux CPU 版本示例
pip install "paddlepaddle>=2.5.0" --extra-index-url https://www.paddlepaddle.org.cn/whl/linux/mkl
# 若使用 GPU，请根据本机 CUDA 版本自行替换为对应的 paddlepaddle-gpu 发行包，例如：
# pip install paddlepaddle-gpu==2.5.2.post120
```

未安装 PaddleOCR 时，程序会在首次尝试识别时给出明确提示。
